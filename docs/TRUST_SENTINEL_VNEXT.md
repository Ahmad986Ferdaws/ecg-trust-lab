...
4. `ABSTAIN`: the signal is valid and supported, but the classifier is not
   sufficiently certain or, only under a non-v1 policy with the opt-in
   coherence gate, its confident per-label decisions form a listed implausible
   combination.
...
The `conformal` package also provides an opt-in
`ClassConditionalLabelwiseConformal` artifact. It calibrates each label's
positive and negative outcomes separately under miss budgets that the caller
must supply, so a rare label's positive coverage is not traded against its
negatives. Its guarantee is per label and outcome, not simultaneous, and says
nothing about error among singleton decisions. Nothing selects it: the Sentinel
