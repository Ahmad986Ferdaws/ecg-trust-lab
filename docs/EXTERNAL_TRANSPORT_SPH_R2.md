# Frozen SPH external-transport protocol r2
...
This r2 protocol supersedes the operational v1 attempt at Git revision
`724a510b03eb539eda2add6de359855f3ffaf2b5` without changing the scientific
contract. The v1 runner completed source checks and the sealed PTB fold-10
clean-equivalence inference, then failed while canonicalizing a nested immutable
source-inventory mapping for the attempt marker. No SPH model inference started,
and no SPH predictions, metrics, or results were produced. The failed root
originally contained only an empty `private/` directory and no automatic attempt
marker.

An operator subsequently appended
`runs/external_transport/sph_figshare_v1/private/PRE_INFERENCE_FAILURE.md`,
then corrected its wording before the r2 freeze to distinguish sealed PTB
clean-equivalence inference from SPH inference. The authoritative local receipt
is 874 bytes with SHA-256
`26cbd6f8a1679faf1ec362712aa63334dd81904666ca4c9de8d8aa5013840a74`.
This is disclosed operator provenance, not a claim that the failed root remained
byte-for-byte unchanged. The sanitized tracked record is
`reports/SPH_PRE_INFERENCE_FAILURES.md`.

The v1 YAML remains unchanged with SHA-256
`6beebf9fc95c90591d5f63b3985ff14fbff403a75862b5690201b1c7d6ff2669`.
The r2 YAML carries a canonical projection hash proving that purpose, source,
labels, signal processing, cohorts, frozen models, runtime, evaluation,
bootstrap, privacy, and reporting language are identical to v1. Only artifact
serialization, failure recording, reservation semantics, checkpoint recovery,
lineage, and the output root changed.
...
recorded cleanly, or the output root already exists, even if it is empty.
...
Outputs belong under `runs/external_transport/sph_figshare_v1__attempt-r2/`. The run is
...
The output root must be absent. Atomic directory creation permanently spends it;
an empty or partial root is a failed attempt and is never reused. Every
post-acquisition exception records its exact phase and whether SPH inference,
prediction generation, checkpointing, metrics, and output commit started or
completed. `output_commit_started` spans the initial outcome-independent
identity/layout commit through the final result commit; it does not mean that
public results exist. A failure-receipt error is attached to, and never masks,
the original exception.

Before SPH inference, r2 atomically commits the protocol snapshot, source
inventory, a hashed complete bound-input snapshot, private cohort
Parquet/alignment, and output layout. The attempt marker binds the same input
list and digest. Immediately
after the one allowed SPH inference pass succeeds—and before any bootstrap or
metric computation—it commits a private, self-hashed inference checkpoint with
targets, all six raw-logit matrices, physical-signal identity, and signal QC.
If downstream metrics or reporting fails, a separately frozen recovery
finalizer must load and verify those exact logits; it must not run SPH model
inference a second time. Immediately before the final derived manifest, the
runner re-hashes every bound input and requires exact equality with the
pre-inference snapshot. A corrected attempt otherwise uses a new immutable
versioned output root and records why it supersedes the earlier one;
