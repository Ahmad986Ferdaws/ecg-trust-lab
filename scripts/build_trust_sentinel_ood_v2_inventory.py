...
class InventoryExpectations:
    ...
    zzu_duration_under_10_seconds: int = EXPECTED_ZZU_EXCLUSION_COUNTS["duration_under_10_seconds"]
    zzu_lead_count_not_12: int = EXPECTED_ZZU_EXCLUSION_COUNTS["lead_count_not_12"]
    zzu_noncanonical_lead_set: int = EXPECTED_ZZU_EXCLUSION_COUNTS["noncanonical_lead_set"]
    ...
    def __post_init__(self) -> None:
        ...
        if exclusions["pediatric_12_lead_flag_false"] != self.zzu_nine_lead_records:
...
def _validate_zzu_selection_counts(
    ...
    if (
        summary.selected_record_count != expectations.zzu_selected_records
        or len(selected) != expectations.zzu_selected_records
    ):
    ...
    if (
        any(record.patient_key is None for record in selected)
        or len(selected_patient_keys) != expectations.zzu_selected_patients
    ):
